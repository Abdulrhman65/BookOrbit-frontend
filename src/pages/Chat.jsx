          ...
          {activeGroup ? (
            <>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div onClick={() => navigate(`/student/${activeGroup.otherStudentId}`)} className="flex items-center gap-3 cursor-pointer group">
                    <div className="w-9 h-9 md:w-10 md:h-10 rounded-xl overflow-hidden bg-white dark:bg-white/5 border border-library-accent/20 shrink-0">
                      {studentImages[activeGroup.otherStudentId] ? <img src={studentImages[activeGroup.otherStudentId]} className="w-full h-full object-cover" alt="" /> : <User size={20} />}
                    </div>
              ...
              <div ref={scrollRef} className="flex-grow overflow-y-auto custom-scrollbar p-3 md:p-6 space-y-6">
                {loadingMessages ? (
                  <div className="flex flex-col items-center justify-center py-20 opacity-20"><Loader className="animate-spin text-library-accent mb-4" /><p className="text-[11px] font-black">جاري تحميل الرسائل...</p></div>
                  ...
                  currentMessages.map((msg) => {
                    ...
                    return (
                      <motion.div key={msg.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`flex ${isMe ? 'justify-start' : 'justify-end'}`}>
                        <div className={`max-w-[90%] md:max-w-[75%] ${isMe ? 'bg-library-primary text-white rounded-[1.25rem] rounded-tr-none' : 'bg-white dark:bg-white/[0.08] backdrop-blur-xl border border-white/10 rounded-[1.25rem] rounded-tl-none shadow-md'} p-3 px-4 shadow-lg`}>
                          <p className="text-[14px] leading-relaxed font-bold">{msg.content}</p>
                          <div className="flex items-center gap-1.5 mt-1.5 justify-end opacity-60">
                            <span className="text-[8px] md:text-[9px] font-black">{new Date(msg.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                            {isMe && (msg.isRead ? <CheckCheck size={12} className="text-emerald-400" /> : <Check size={12} />)}
                          </div>
                        </div>
                      </motion.div>
                    );
                  ...
                  <div className="flex flex-col items-center justify-center py-20 opacity-20"><MessageSquare size={40} className="mb-4" /><p className="text-[11px] font-black">لا توجد رسائل بعد. ابدأ الدردشة الآن...</p></div>
                )}
              </div>
              ...
              <div className="p-3 md:p-5 bg-white/60 dark:bg-black/40 backdrop-blur-3xl border-t border-library-primary/5 dark:border-white/5 relative z-50">
                ...
                <form onSubmit={handleSendMessage} className="flex items-center gap-2 md:gap-3 max-w-5xl mx-auto">
                  ...
                  <motion.button whileTap={{ scale: 0.9 }} type="submit" disabled={(!message.trim() && !selectedFile)} className={`shrink-0 w-11 h-11 rounded-xl flex items-center justify-center transition-all ${message.trim() || selectedFile ? 'bg-library-primary text-white shadow-lg' : 'bg-gray-100 dark:bg-white/5 text-gray-400'}`}><Send size={18} /></motion.button>
                </form>
              </div>
            </>
          ) : (
